...
fn main() {
    ...
    println!("Enter staff role (as it is listed i.e no all-lowercase or extra spaces:");
    io::stdin().read_line(&mut role).expect("Failed to read smth");
    let role = role.trim();
    ...
    let years: u32 = years.trim().parse().expect("Invalod input");
